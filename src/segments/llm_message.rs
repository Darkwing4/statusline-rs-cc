...
pub use crate::config_schema::LlmMessage;
use crate::segments::{GitCache, Segment};
use crate::statusline_cache_dir::cache_dir;

pub const REFRESH_FLAG: &str = "--llm-refresh";

const ELLIPSIS: char = '…';

impl Segment for LlmMessage {
    fn render(&self, _json: &Value, _git: &mut GitCache) -> Option<String> {
        if self.command.is_empty() {
            return None;
        }

        let paths = CachePaths::for_segment(self)?;
...
impl LlmMessage {
    ...
    fn fingerprint(&self) -> String {
        ...
        for part in std::iter::once(self.command.as_str())
            ...
            .chain(std::iter::once(self.prompt.as_str()))
        {
    ...
    fn run_command(&self) -> Option<String> {
        ...
        if let Some(mut stdin) = child.stdin.take() {
            let _ = stdin.write_all(self.prompt.as_bytes());
        }
...
pub fn refresh(fingerprint: &str) {
    ...
    let Some(segment) = config
        ...
        .filter_map(|spec| match spec {
            crate::config::SegmentSpec::LlmMessage(segment) => Some(segment),
            _ => None,
        })
    ...
    let Some(paths) = CachePaths::for_segment(&segment) else {
        return;
    };

    let Some(text) = segment.run_command() else {
        return;
    };
...
impl CachePaths {
    fn for_segment(segment: &LlmMessage) -> Option<Self> {
        let dir = cache_dir()?;
        let fingerprint = segment.fingerprint();

        Some(CachePaths {
            result: dir.join(format!("llm-{fingerprint}.txt")),
            attempt: dir.join(format!("llm-{fingerprint}.attempt")),
            pending: dir.join(format!("llm-{fingerprint}.pending")),
        })
    }

    fn mark_attempt(&self) {
        if let Some(parent) = self.attempt.parent() {
    ...
    fn store(&self, text: &str) {
        if let Some(parent) = self.result.parent() {
            if fs::create_dir_all(parent).is_err() {
                return;
            }
        }
...
#[cfg(test)]
mod tests {
    use super::{is_expired, last_non_empty_line, sanitize, LlmMessage};
    use crate::types::Color;

    fn segment() -> LlmMessage {
        LlmMessage {
            color: Color::Named(90),
            prefix: "> ".to_string(),
            command: "codex".to_string(),
            args: vec!["exec".to_string()],
            prompt: "one quote".to_string(),
            ttl_seconds: 900,
            max_chars: 80,
        }
    }
    ...
    #[test]
    fn fingerprint_tracks_command_args_and_prompt() {
        let base = segment();
        let mut other_prompt = segment();
        other_prompt.prompt = "another quote".to_string();
        let mut other_args = segment();
        other_args.args = vec!["exec".to_string(), "--json".to_string()];

        assert_eq!(base.fingerprint(), segment().fingerprint());
